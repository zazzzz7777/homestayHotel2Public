		...
		<el-form :inline="true" :model="searchQuery" class="list_search">
			<div class="search_view">
				...
				<div class="search_box">
					<el-input class="search_inp" v-model="searchQuery.yudingbianhao" placeholder="预订编号"
						clearable>
					</el-input>
				</div>
			</div>
			<div class="search_btn_view">
				...
				<el-button class="add_btn" type="success" v-if="btnAuth('tuifangxinxi','新增')" @click="addClick">新增</el-button>
			</div>
		</el-form>
			...
			<div class="data_box">
				<div class="data_view">
					<el-table v-loading="listLoading" class="data_table" :data="list" border :row-style="{'cursor':'pointer'}"
						...
						<el-table-column label="预订编号" :resizable='true' align="left" header-align="left">
							<template #default="scope">
								{{scope.row.yudingbianhao}}
							</template>
						</el-table-column>
						<el-table-column label="房间号" :resizable='true' align="left" header-align="left">
							<template #default="scope">
								{{scope.row.fangjianhao}}
							</template>
						</el-table-column>
						<el-table-column label="客房类型" :resizable='true' align="left" header-align="left">
							<template #default="scope">
								{{scope.row.kefangleixing}}
							</template>
						</el-table-column>
						<el-table-column label="入住日期" :resizable='true' align="left" header-align="left">
							<template #default="scope">
								{{scope.row.ruzhuriqi}}
							</template>
						</el-table-column>
						<el-table-column label="入住天数" :resizable='true' align="left" header-align="left">
							<template #default="scope">
						...
						<el-table-column label="退房时间" :resizable='true' align="left" header-align="left">
							<template #default="scope">
								{{scope.row.tuifangshijian}}
							</template>
						</el-table-column>
						<el-table-column label="客房部账号" :resizable='true' align="left" header-align="left">
							<template #default="scope">
								{{scope.row.kefangbuzhanghao}}
							</template>
						</el-table-column>
					</el-table>
				</div>
...
<script setup>
	...
	const addClick = () => {
		router.push('/index/tuifangxinxiAdd')
	}
	//判断是否从个人中心跳转
	...
	const getList = () => {
		...
		if(searchQuery.value.yudingbianhao&&searchQuery.value.yudingbianhao!=''){
			params.yudingbianhao = '%' + searchQuery.value.yudingbianhao + '%'
		}
		context?.$http({
	...
	const tableDetailClick = (row) => {
		router.push('tuifangxinxiDetail?id=' + row.id + (centerType.value?'&&centerType=1':''))
	}
	//下载文件
	const download = (file) =>{
		if(!file){
